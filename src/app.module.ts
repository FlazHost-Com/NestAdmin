  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => {
        ...
        const dbName = config.get<string>('DB_DATABASE') || config.get<string>('DB_NAME', 'nestadmin.sqlite')
        ...
        if (isSqliteLike) {
          return { ...baseOpts, type: dbType, database: isMemory ? ':memory:' : join(process.cwd(), dbName) } as any
        }
        return {
          ...baseOpts,
          type: dbType,
          ...
          username: (config.get<string>('DB_USERNAME') || config.get<string>('DB_USER', 'root')) as string,
          password: (config.get<string>('DB_PASSWORD') || config.get<string>('DB_PASS', '')) as string,
          database: (config.get<string>('DB_DATABASE') || dbName) as string,
        } as any
      },
    }),
