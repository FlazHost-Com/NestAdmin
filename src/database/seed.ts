...
async function seed() {
  ...
  if (existingSetting.length === 0) {
    await em.query(
      ...
      [uuidv4(), 'NA', 'NestAdmin', 'NestAdmin - Bootstrap Admin Panel', 'Blue', 'agency-consulting-002-creative-agency']
    )
  ...
  let roleId: string
  const existingRole = await em.query("SELECT id FROM roles WHERE name = 'Administrator' LIMIT 1")
  ...
  const existingUser = await em.query("SELECT id FROM users WHERE email = 'admin@admin.com' LIMIT 1")
  if (existingUser.length === 0) {
    const userId = uuidv4()
    const hashedPwd = await bcrypt.hash('12345678', parseInt(process.env.BCRYPT_ROUNDS || '10'))
    await em.query(
      ...
      [userId, '0000000001', 'Administrator', '12345678910', 'admin@admin.com', hashedPwd, 'Active', false, '', 'Asia/Jakarta']
    )
    await em.query(
