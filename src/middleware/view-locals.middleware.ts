...
export class ViewLocalsMiddleware implements NestMiddleware {
  constructor(
    @Optional() @Inject(SettingCacheService) private settingCache?: SettingCacheService,
  ) {}
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    res.locals.addOrUpdateQueryParam = (url: string, key: string, value: any) => {
      try {
    ...
    res.locals.hasAccess = (routeName: string, method: string): boolean => {
      ...
      const perms: Array<{ name: string; method: string }> = user.permissions || []
      return perms.some(
        p => p.name === routeName && p.method?.toUpperCase() === method.toUpperCase(),
      )
    }
