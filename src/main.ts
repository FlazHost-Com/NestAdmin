...
async function bootstrap() {
  ...
  if (isProd) {
    ...
    if (!sess || sess.length < 16) throw new Error('SESSION_SECRET required in production')
    if (!jwt || jwt.length < 16) throw new Error('JWT_SECRET required in production')
  }
  ...
  const server = await app.listen(port, () => {
    console.log(`NestAdmin running on http://localhost:${port} (mode: ${appMode})`)
  })
