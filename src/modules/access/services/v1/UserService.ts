...
import { paginate, ciLike, removePrefix, removeEmptyFields, generateCode } from '../../../../helpers/functions'
import { AppError, NotFoundError } from '../../../../errors/AppError'

const TIMEZONES = [
  'UTC',
  'Asia/Jakarta', 'Asia/Makassar', 'Asia/Jayapura',
  'Asia/Singapore', 'Asia/Kuala_Lumpur', 'Asia/Bangkok', 'Asia/Ho_Chi_Minh',
  'Asia/Manila', 'Asia/Hong_Kong', 'Asia/Taipei', 'Asia/Shanghai', 'Asia/Tokyo',
  'Asia/Seoul', 'Asia/Kolkata', 'Asia/Karachi', 'Asia/Dubai', 'Asia/Riyadh',
  'Africa/Nairobi', 'Europe/Istanbul', 'Europe/Moscow',
...
export class UserService implements IUserService {
  ...
  async index(filter: any) {
    ...
    if (clean.code)   query = query.andWhere(...ciLike('users.code',  'code',  clean.code))
    if (clean.name)   query = query.andWhere(...ciLike('users.name',  'name',  clean.name))
    if (clean.phone)  query = query.andWhere(...ciLike('users.phone', 'phone', clean.phone))
    if (clean.email)  query = query.andWhere(...ciLike('users.email', 'email', clean.email))
    if (clean.status) query = query.andWhere('users.status = :status', { status: clean.status })
    if (clean.role)   query = query.andWhere('roles.id = :roles_id',  { roles_id: clean.role })
  ...
  async store(request: any, files: any = null) {
    const id = uuidv4()
    const roles = await this.roleRepo.findBy({ id: In(Array.isArray(request['roles[]']) ? request['roles[]'] : request.roles || []) })
    if (!roles.length) {
  ...
  async edit(id: string) {
    const roles = await this.roleRepo.find()
    const data = await this.userRepo.findOne({ where: { id }, relations: { roles: true } })
