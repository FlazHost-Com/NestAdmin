...
export class RoleService implements IRoleService {
  ...
  async index(filter: any) {
    ...
    if (clean.name)   query = query.andWhere(...ciLike('roles.name', 'name', clean.name))
    if (clean.desc)   query = query.andWhere(...ciLike('roles.desc', 'desc', clean.desc))
    if (clean.status) query = query.andWhere('roles.status = :status', { status: clean.status })
  ...
  async update(id: string, request: any) {
    const conflict = await this.roleRepo.findOne({ where: { id: Not(id), name: request.name } })
  ...
  async permission(role_id: string, filter: any) {
    const clean = removePrefix(filter, 'q_')
    const role = await this.roleRepo.findOne({ where: { id: role_id }, relations: { permissions: true } })
    let query = this.permissionRepo.createQueryBuilder('permissions')

    if (clean.name)   query = query.andWhere(...ciLike('permissions.name', 'name', clean.name))
    if (clean.method) query = query.andWhere('permissions.method = :method', { method: clean.method })
    if (clean.desc)   query = query.andWhere(...ciLike('permissions.desc', 'desc', clean.desc))
    if (clean.status) {
  ...
  async permissionAssign(role_id: string, permission_id: string) {
    const role = await this.roleRepo.findOne({ where: { id: role_id }, relations: { permissions: true } })
    if (!role) throw new NotFoundError('Role not found')
    const permission = await this.permissionRepo.findOne({ where: { id: permission_id } })
  ...
  async permissionUnassignSelected(role_id: string, permissions: string[]) {
    const role = await this.roleRepo.findOne({ where: { id: role_id }, relations: { permissions: true } })
    if (!role) throw new NotFoundError('Role not found')
    role.permissions = role.permissions.filter(p => !permissions.includes(p.id))
    const result = await this.roleRepo.save(role)
    if (!result) throw new AppError('Unassign Permission Fail', 500)
    return result
  }
