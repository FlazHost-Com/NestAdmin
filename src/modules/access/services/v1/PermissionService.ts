...
export class PermissionService implements IPermissionService {
  ...
  async index(filter: any) {
    ...
    if (clean.status)     query = query.andWhere('permissions.status = :status',         { status: clean.status })
    if (clean.guard)      query = query.andWhere('permissions.guard_name = :guard',      { guard: clean.guard })
    if (clean.guard_name) query = query.andWhere('permissions.guard_name = :guard_name', { guard_name: clean.guard_name })
  ...
  async store(request: any) {
    const find = await this.permissionRepo.findOne({ where: { name: request.name } })
  ...
  async update(id: string, request: any) {
    const conflict = await this.permissionRepo.findOne({ where: { id: Not(id), name: request.name } })
  ...
  async syncFromRoutes() {
    ...
    for (const route of routes) {
      const existing = await this.permissionRepo.findOne({
        where: { name: route.name, method: route.method, guard_name: route.guardName },
      })
      if (!existing) {
