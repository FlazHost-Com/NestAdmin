...
export class ProfileService implements IProfileService {
  ...
  async index(userId: string): Promise<any> {
    const data = await this.userRepo.findOne({ where: { id: userId }, relations: { roles: true } })
    if (!data) throw new NotFoundError('User not found')
    return { data, timezones: TIMEZONES }
  }

  async update(userId: string, request: any, files?: Record<string, any[]>): Promise<User> {
