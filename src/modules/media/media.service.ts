...
export class MediaService {
  ...
  private validateMagicBytes(buffer: Buffer, mimeType: string): boolean {
    ...
    return checks.every(([offset, hex]) => {
      const slice = buffer.slice(offset, offset + hex.length / 2).toString('hex')
      return slice === hex
    })
  }
  ...
  public upload(
    ...
    if (buffer.length > MAX_SIZE_BYTES) {
      throw new AppError(`Ukuran file melebihi batas ${MAX_SIZE_BYTES / 1024 / 1024} MB`, 400)
    }
    ...
    const ext = extMap[mimetype] || path.extname(originalname).toLowerCase() || '.bin'
