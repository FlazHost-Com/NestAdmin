...
export class AuthWebController {
  constructor(private authService: AuthService) {
    ...
    routeRegistry.register('admin.v1.auth.reset.req', 'GET', '/admin/v1/auth/reset/req')
    routeRegistry.register('admin.v1.auth.reset.request', 'POST', '/admin/v1/auth/reset/request')
    routeRegistry.register('admin.v1.auth.reset.proc', 'GET', '/admin/v1/auth/reset/proc')
    routeRegistry.register('admin.v1.auth.reset.process', 'POST', '/admin/v1/auth/reset/process')
  }
  ...
  registerPage(@Req() req: Request, @Res() res: Response) {
    res.render('auth/views/be/default/register', { layout: 'layouts/be/default/full-width', title: 'Register', flash: (req as any).flash?.() })
  }
  ...
  resetReqPage(@Req() req: Request, @Res() res: Response) {
    res.render('auth/views/be/default/reset_req', { layout: 'layouts/be/default/full-width', title: 'Reset Password', flash: (req as any).flash?.() })
  }
  ...
  async resetRequest(@Req() req: Request, @Res() res: Response) {
    ...
    res.redirect(`/admin/v1/auth/reset/proc?email=${encodeURIComponent(email)}`)
  }

  @Get('/admin/v1/auth/reset/proc')
  resetProcPage(@Req() req: Request, @Res() res: Response) {
    res.render('auth/views/be/default/reset_proc', { layout: 'layouts/be/default/full-width', title: 'Enter OTP', email: req.query.email, flash: (req as any).flash?.() })
  }
